...
const validateField = (field: FieldConfig, value: any) => {
  ...
  if (field.validations?.minLength && value?.length < Number(field.validations.minLength.value)) {
    errors.push(
      field.validations.minLength.errorMessage ||
        `Minimum length is ${field.validations.minLength.value}`
    );
  }
  if (field.validations?.maxLength && value?.length > Number(field.validations.maxLength.value)) {
    errors.push(
  ...
  if (field.validations?.email && value && !/^\S+@\S+\.\S+$/.test(value)) {
    errors.push(
      field.validations.email.errorMessage || "Invalid email address"
    );
  }
  if (
    field.validations?.passwordRule &&
    value &&
    !(field.validations.passwordRule.value as RegExp).test(value)
  ) {
    errors.push(
      field.validations.passwordRule.errorMessage || "Invalid password format"
    );
  }
          ...
          if (f.parentIds?.includes(id) && f.formula && f.formula.trim() !== "") {
            try {
              ...
              if (f.formula === "AGE_FROM_DOB") {
                // Get the DOB value from the parent field that was changed
                const dobValue = updated[id] || prev[id];
                console.log("DOB Value:", dobValue, "Field ID:", id, "Updated:", updated); // Debug log
                if (dobValue && dobValue !== "") {
                  ...
                  if (!isNaN(dob.getTime())) { // Check if valid date
                    ...
                    result = monthDiff < 0 || (monthDiff === 0 && today.getDate() < dob.getDate()) 
                      ? age - 1 : age;
                    console.log("Calculated Age:", result); // Debug log
                  } else {
                    result = "";
                  }
                } else {
                ...
                // Get values from parent fields dynamically
                const nameParts = f.parentIds?.map(parentId => 
                  updated[parentId] || prev[parentId] || ""
                ).filter(part => part !== "") || [];
      ...
      <Paper elevation={3} sx={{ p: 4, borderRadius: 3 }}>
        <Box sx={{ mb: 4, textAlign: 'center' }}>
          ...
          <Typography variant="body1" color="text.secondary">
            Test your form below. Fill in the fields and click "Validate" to check for errors.
          </Typography>
        </Box>
        ...
        <Stack spacing={3}>
          {fields.map((field) => {
            ...
            switch (field.type) {
              ...
              case "checkbox":
                return (
                  <Box key={field.id} sx={{ p: 3, border: '1px solid', borderColor: 'grey.200', borderRadius: 2, backgroundColor: 'grey.50' }}>
